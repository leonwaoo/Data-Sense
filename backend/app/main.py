...
app = FastAPI(
    title="DataSense API",
    description="API para upload, perfil, qualidade e perguntas analiticas sobre planilhas e dados tabulares.",
    version="0.1.0",
)
...
async def upload_dataset(file: UploadFile = File(...)) -> dict:
    if not file.filename:
        raise HTTPException(status_code=400, detail="Envie um arquivo valido.")

    content = await file.read()
    try:
        dataset = load_dataset(content=content, file_name=file.filename)
